...
void send_to_mqtt(void * parameter) { // NOSONAR
  for (;;) {
        ...
        if ( Pow_mqtt_send > 5 ) {
          ...
          float wattheure = (timemesure * abs(gDisplayValues.watt) / timemilli);  
            ...
            if (configmqtt.HA) {
              ...
              switch_dimmerlocal.sendInt(config.dimmerlocal ? 1 : 0); //ajout dimmerlocal
    ...
    // Appel de client.loop() pour traiter les messages MQTT entrants
    // CRITIQUE: Cet appel doit être HORS de tous les blocs conditionnels
    // pour que les commandes Home Assistant (Relay2, DimmerLocal) soient traitées immédiatement
    #ifndef LIGHT_FIRMWARE
    if (config.mqtt && (WiFi.status() == WL_CONNECTED) && client.connected()) {
      client.loop(); // on envoie ce qui est en attente et on traite les messages entrants
    }
    #endif
